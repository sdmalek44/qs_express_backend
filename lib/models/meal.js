...
class Meal {
  ...
  static all(){
    return database('meals')
      ...
      .select('meals.id', 'meals.name', 'meal_foods.food_id',
        'foods.name as food_name', 'foods.calories', 'meal_foods.created_at',
        'meal_foods.updated_at')
      .orderBy('meals.id')
        .then((meals) => {
...
function formatFood(meal){
  return {
            ...
            added: meal.created_at,
            changed: meal.updated_at
          }
}
